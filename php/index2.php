...
$host = '146.148.55.202';
$port = '5432';
$dbname = 'Asesor_prueba';
$user = 'mastersito';
$password = 'pD0V7/Za94.p';
$sslmode = 'prefer';
...
try {
    ...
    // Establecer atributos de error para manejar excepciones
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
      ...
      $sql = "SELECT * FROM res_users WHERE login = ?";
      $stmt = $conn->prepare($sql);
      $stmt->bind_param("s", $userEmail); // El mismo parámetro para username o email
      $stmt->execute();
      ...
      if ($result->num_rows > 0) {
          ...
          if (password_verify($password, $row['password'])) {
              ...
              $_SESSION['username'] = $row['login']; // Nombre de usuario
              $_SESSION['admin'] = $row['is_admin_presp']; // Rol de administrador o no
